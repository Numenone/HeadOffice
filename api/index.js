...
<head>
    ...
    <style>
        ...
        .card-disabled button { cursor: not-allowed !important; background-color: rgba(30, 41, 59, 0.5) !important; color: #64748b !important; border-color: rgba(255, 255, 255, 0.1) !important; }
        .card-disabled button:hover { background-color: rgba(30, 41, 59, 0.5) !important; }
...
<body class="min-h-screen bg-grid">
    ...
    <script>
            ...
            try {
                ...
                data.forEach(emp => {
                    ...
                    const buttonContent = isDisabled 
                        ? '<i data-lucide="link-2-off" class="w-3 h-3"></i> SEM LINK' 
                        : \`<i data-lucide="zap" class="w-3 h-3"></i> \${emp.resumo ? 'REPROCESSAR DADOS' : 'INICIAR ANÁLISE'}\`;

                    grid.innerHTML += \`
                    <div id="card-\${emp.id}" class="glass-card rounded-2xl flex flex-col h-full relative group company-card \${disabledClass}" data-status="\${emp.status_cliente || 'Neutro'}">
                        <div class="p-5 pb-3 border-b border-white/5">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                \${emp.doc_link ? \`<a href="\${emp.doc_link}" target="_blank" class="text-slate-600 hover:text-white transition-colors p-1 hover:bg-white/10 rounded"><i data-lucide="file-text" class="w-4 h-4"></i></a>\` : '<span class="text-slate-700 p-1"><i data-lucide="file-x" class="w-4 h-4"></i></span>'}
                            </div>
                            <div class="flex gap-2">
                                <span class="badge st-\${sCli}">\${emp.status_cliente || 'PENDENTE'}</span>
                            </div>
                        </div>
                        <div id="content-\${emp.id}" class="p-5 flex-grow text-sm">
                            \${contentHtml}
                        </div>
        ...
        async function summarize(nome, id) {
            ...
            if (cardContent) {
                cardContent.innerHTML = '<div class="flex flex-col items-center justify-center h-40 text-slate-500 gap-2 animate-pulse"><i data-lucide="bot" class="w-8 h-8 text-indigo-400"></i><p class="text-xs font-mono">IA PENSANDO...</p></div>';
                lucide.createIcons();
            }
            ...
            try {
                ...
                if (!res.ok || !json.success) {
                    throw new Error(json.details || json.error || 'Falha na API');
                }

                // SUCCESS
                const card = document.getElementById('card-' + id);
                if (cardContent) cardContent.innerHTML = json.data.resumo;
                
                if (card) {
                    card.setAttribute('data-status', json.data.status_cliente);
                    const badge = card.querySelector('.badge');
                    if (badge) {
                        const newStatusClass = (json.data.status_cliente || 'Neutro').replace(/\s/g, '');
                        badge.className = \`badge st-\${newStatusClass}\`;
                        badge.textContent = json.data.status_cliente || 'PENDENTE';
                    }
                    
                    const lastUpdatedEl = card.querySelector('.font-mono.text-slate-500');
                    if(lastUpdatedEl) {
                        lastUpdatedEl.textContent = new Date(json.data.last_updated).toLocaleDateString();
                    }
                }
                
                btn.disabled = false;
                btn.className = originalBtnClass;
                btn.innerHTML = '<i data-lucide="zap" class="w-3 h-3"></i> REPROCESSAR DADOS';
                lucide.createIcons();

            } catch (e) {
                alert('Erro ao processar: ' + e.message);
                btn.innerHTML = '<i data-lucide="alert-triangle" class="w-3 h-3"></i> FALHA';
                btn.className = "w-full bg-red-900/20 text-red-400 border border-red-500/30 py-2.5 rounded-lg text-xs font-bold flex justify-center items-center gap-2";
                lucide.createIcons();
